            ...
            osmosis
                ...
                .data(function(listing) {
                    ...
                    mongoObj
                        ...
                        .add("propertyDescription", listing.propertyDescription.join(" "))
                        .add("price", listing.price)
                        .add("bathrooms", stringConstants.BATHROOM_STATUS_UNAVAILABLE)
                        .add("address", listing.address)
                        .add("noOfRooms", listing.rooms)
                        .add("sharingStatus", listing.sharingStatus)
                        .add("roomFacilities", listing.roomFacilities.map(item => item.substring(2)))
                        .add("nearby", listing.nearby.map(item => item.substring(2)))
                        .add("images", listing.imageSource)
                        .add("galleryImages", galleryImages)
                        .add("imagesDirectory", '/home/deepanshu/xacco/xacco_node_scrapper/images/coveLivings/' +
                            listing.propertyName.split("")
                                .map(item => item.toLowerCase()[0].toUpperCase() + item.toLowerCase().substring(1)).join()
                            + '_' + listing.price  + '/roomImages')
                        .add("galleryImagesDirectory", '/home/deepanshu/xacco/xacco_node_scrapper/images/coveLivings/' +
                            listing.propertyName.split(" ")
                                .map(item => item.toLowerCase()[0].toUpperCase() + item.toLowerCase().substring(1)).join()
                            + '_' + listing.price  + '/galleryImages')
                    .save();
                })
            .log(console.log)
            // .error(console.log)
